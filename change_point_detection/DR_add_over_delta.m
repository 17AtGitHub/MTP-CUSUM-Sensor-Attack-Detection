...
W_common = mvnrnd(zeros(2,1), 0.1 * eye(2), change_point)'; % Actuation noise W ~ N(0, 0.1I)
V_common = sqrt(0.1) * randn(1, change_point); % Observation noise V ~ N(0, 0.1)

% Generate noise after the change point
W_with_change_after = mvnrnd(zeros(2,1), 0.1 * eye(2), T - change_point)'; % No change after change point
V_with_change_after = sqrt(0.1) * randn(1, T - change_point) + 0.5; % Mean shift after change point
W_no_change_after = mvnrnd(zeros(2,1), 0.1 * eye(2), T - change_point)'; % No change after change point
V_no_change_after = sqrt(0.1) * randn(1, T - change_point); % No change after change point
