...
num_timesteps = 150;
attack_start = 100;
attack_magnitude = 2.9;
tau_values = linspace(10, 8, 70);
bias = 2.7; % Fixed bias
...
for sim = 1:num_simulations
    [fa_counts] = run_simulation(tau_values, false, num_timesteps, ...
        attack_start, attack_magnitude, bias, F, G, C, K, R1, R2, L, Sigma);
...
title('ROC Curve for Attack Detection Model with Quadratic Residual');
grid on;
...
% Function to run a single simulation with quadratic residuals
function [alarm_counts] = run_simulation(tau_values, with_attack, num_timesteps, attack_start, attack_magnitude, bias, F, G, C, K, R1, R2, L, Sigma)
